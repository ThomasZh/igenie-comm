...
public class SyncTaskMemberReq
		extends ReqCommand
{
	public SyncTaskMemberReq()
	{
		super();
	...
	public SyncTaskMemberReq(String taskId, short version)
	{
		this();
	...
	@Override
	public SyncTaskMemberReq decode(TlvObject tlv)
			throws UnsupportedEncodingException
	{
		logger.info("from tlv:(tag=" + Command.SYNC_TASKPRO_MEMBER_REQ + ", child=3) to command");
			...
			throws UnsupportedEncodingException
	{
		...
		TlvObject tlv = new TlvObject(Command.SYNC_TASKPRO_MEMBER_REQ);
		tlv.push(tSequence);
		tlv.push(tTaskId);
		tlv.push(tVersion);

		logger.info("from command to tlv package:(tag=" + Command.SYNC_TASKPRO_MEMBER_REQ + ", child=3, length="
				+ tlv.getLength() + ")");
	...
	private final static Logger logger = LoggerFactory.getLogger(SyncTaskMemberReq.class);
