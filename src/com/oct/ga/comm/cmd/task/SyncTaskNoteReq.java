...
public class SyncTaskNoteReq
		extends ReqCommand
{
	public SyncTaskNoteReq()
	{
		super();
	...
	public SyncTaskNoteReq(String taskId, int version)
	{
		this();
	...
	@Override
	public SyncTaskNoteReq decode(TlvObject tlv)
			throws UnsupportedEncodingException
	{
		logger.info("from tlv:(tag=" + Command.SYNC_TASKPRO_NOTE_REQ + ", child=3) to command");
			...
			throws UnsupportedEncodingException
	{
		...
		TlvObject tlv = new TlvObject(Command.SYNC_TASKPRO_NOTE_REQ);
		tlv.push(tSequence);
		tlv.push(tTaskId);
		tlv.push(tVersion);

		logger.info("from command to tlv package:(tag=" + Command.SYNC_TASKPRO_NOTE_REQ + ", child=3, length="
				+ tlv.getLength() + ")");
	...
	private final static Logger logger = LoggerFactory.getLogger(SyncTaskNoteReq.class);
