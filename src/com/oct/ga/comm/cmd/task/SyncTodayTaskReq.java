...
public class SyncTodayTaskReq
		extends ReqCommand
{
	public SyncTodayTaskReq()
	{
		super();
	...
	public SyncTodayTaskReq(int lastTryTime)
	{
		this();
	...
	@Override
	public SyncTodayTaskReq decode(TlvObject tlv)
			throws UnsupportedEncodingException
	{
		logger.info("from tlv:(tag=" + Command.SYNC_TODAY_TASK_REQ + ", child=2) to command");
			...
			throws UnsupportedEncodingException
	{
		...
		TlvObject tlv = new TlvObject(Command.SYNC_TODAY_TASK_REQ);
		tlv.push(tSequence);
		tlv.push(tLastTryTime);

		logger.info("from command to tlv package:(tag=" + Command.SYNC_TODAY_TASK_REQ + ", child=2, length="
				+ tlv.getLength() + ")");
	...
	private final static Logger logger = LoggerFactory.getLogger(SyncTodayTaskReq.class);
