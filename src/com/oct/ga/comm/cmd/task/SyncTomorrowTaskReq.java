...
public class SyncTomorrowTaskReq
		extends ReqCommand
{
	public SyncTomorrowTaskReq()
	{
		super();
	...
	public SyncTomorrowTaskReq(int lastTryTime)
	{
		this();
	...
	@Override
	public SyncTomorrowTaskReq decode(TlvObject tlv)
			throws UnsupportedEncodingException
	{
		logger.info("from tlv:(tag=" + Command.SYNC_TOMORROW_TASK_REQ + ", child=2) to command");
			...
			throws UnsupportedEncodingException
	{
		...
		TlvObject tlv = new TlvObject(Command.SYNC_TOMORROW_TASK_REQ);
		tlv.push(tSequence);
		tlv.push(tLastTryTime);

		logger.info("from command to tlv package:(tag=" + Command.SYNC_TOMORROW_TASK_REQ + ", child=2, length="
				+ tlv.getLength() + ")");
	...
	private final static Logger logger = LoggerFactory.getLogger(SyncTomorrowTaskReq.class);
