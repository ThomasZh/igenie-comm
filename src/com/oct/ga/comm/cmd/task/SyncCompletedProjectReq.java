...
public class SyncCompletedProjectReq
		extends ReqCommand
{
	public SyncCompletedProjectReq()
	{
		super();
	...
	public SyncCompletedProjectReq(int lastTryTime)
	{
		this();
	...
	@Override
	public SyncCompletedProjectReq decode(TlvObject tlv)
			throws UnsupportedEncodingException
	{
		logger.info("from tlv:(tag=" + Command.SYNC_PROJECT_COMPLETED_REQ + ", child=2) to command");
			...
			throws UnsupportedEncodingException
	{
		...
		TlvObject tlv = new TlvObject(Command.SYNC_PROJECT_COMPLETED_REQ);
		tlv.push(tSequence);
		tlv.push(tLastTryTime);

		logger.info("from command to tlv package:(tag=" + Command.SYNC_PROJECT_COMPLETED_REQ + ", child=2, length="
				+ tlv.getLength() + ")");
	...
	private final static Logger logger = LoggerFactory.getLogger(SyncCompletedProjectReq.class);
