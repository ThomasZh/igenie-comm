...
public class SyncUncompletedProjectReq
		extends ReqCommand
{
	public SyncUncompletedProjectReq()
	{
		super();
	...
	public SyncUncompletedProjectReq(int lastTryTime)
	{
		this();
	...
	@Override
	public SyncUncompletedProjectReq decode(TlvObject tlv)
			throws UnsupportedEncodingException
	{
		logger.info("from tlv:(tag=" + Command.SYNC_PROJECT_UNCOMPLETED_REQ + ", child=2) to command");
			...
			throws UnsupportedEncodingException
	{
		...
		TlvObject tlv = new TlvObject(Command.SYNC_PROJECT_UNCOMPLETED_REQ);
		tlv.push(tSequence);
		tlv.push(tLastTryTime);

		logger.info("from command to tlv package:(tag=" + Command.SYNC_PROJECT_UNCOMPLETED_REQ + ", child=2, length="
				+ tlv.getLength() + ")");
	...
	private final static Logger logger = LoggerFactory.getLogger(SyncUncompletedProjectReq.class);
